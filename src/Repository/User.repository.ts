...
export class UserRepository {
	...
	static async findUserByPhone(phoneNo: string, transaction?: Transaction): Promise<UserAttributes | null> {
		return await UserModel.findOne({
			where: { phoneNo },
			// include: [{ model: RoleModel, as: 'roleData' }],
			transaction
	...
	static async update({ email, isActive, isDeleted, payload }: { email: string; isActive: boolean; isDeleted: boolean, payload: any }) {
		let criteria = { isActive, isDeleted }
		return await UserModel.update({ ...criteria,  ...payload }, { where: { email } });
	}

	static async countByRole(roleId: string, transaction?: Transaction): Promise<number> {
		return await UserModel.count({ where: { role: roleId }, transaction });
	}
}
